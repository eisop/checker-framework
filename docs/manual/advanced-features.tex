...
If you supply the \<-AassumeAssertions=enabled> command-line option, then
the Checker Framework assumes assertions (that is, Java \<assert>
statements) are enabled, as if Java is run with the \<-ea> or
\<-enableassertions> command-line argument.  If you supply
\<-AassumeAssertions=disabled>, then the Checker
Framework assumes assertions are disabled, as if Java is run with the
\<-da> or \<-disableassertions> command-line argument.
\<-AassumeAssertions=neither> is the default behavior described above.  It is
uncommon to supply the option.

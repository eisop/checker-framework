...
Alternatively, the \<-AuseOptimisticDefaultsForUncheckedCode=bytecode> option applies
optimistic defaults to unchecked bytecode.  Method parameters and upper bounds default to top;
...
Optimistic defaults reduce false positives but are unsound: they trust unchecked code to accept
...
(\code{source} or \code{bytecode}) cannot use both optimistic and conservative defaults.
...
to an unannotated field but reports its reads; the optimistic bottom qualifier accepts

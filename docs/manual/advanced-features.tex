...
Locations specified in \refqualclass{framework/qual}{DefaultQualifier} must be
permitted by the qualifier's \refqualclass{framework/qual}{TargetLocations}
meta-annotation.  If \code{@DefaultQualifier} specifies a location prohibited
by the qualifier's \code{@TargetLocations} (or if \code{locations} is omitted
and the qualifier cannot be applied to all locations), the Checker Framework
issues a compile-time error.  For example, bottom qualifiers typically
restrict their target locations to lower and upper bounds (preventing
programmers from writing them explicitly on parameters, return types, or
fields); therefore, attempting to set a bottom qualifier as a default on
those locations via \code{@DefaultQualifier} is an error.
...
Alternatively, the \<-AusePermissiveDefaultsForUncheckedCode=bytecode> option applies
permissive defaults to unchecked bytecode.  Method parameters and upper bounds default to top;
method returns, fields, and lower bounds default to bottom.  For example, the method above is
interpreted as:
...
Permissive defaults reduce false positives but are unsound: they trust unchecked code to accept
all inputs and return values satisfying every requirement.  When enabled for source code,
they suppress warnings in unannotated code, like conservative defaults.  A kind of code
(\code{source} or \code{bytecode}) cannot use both permissive and conservative defaults.

Fields are the one location where neither choice is right, because a single default
cannot suit both reads and writes.  The conservative top qualifier accepts every write
to an unannotated field but reports its reads; the permissive bottom qualifier accepts
every read but reports writes, so \<u.field = x> is rejected unless \<x> itself has the
bottom qualifier.  Serving both would require defaulting a field differently for reads
and for writes, which the Checker Framework does not yet do (see the discussion below).
Until then, annotate the field (see Chapter~\ref{annotating-libraries}) or suppress the
warning (see Chapter~\ref{suppressing-warnings}).
...
% LocalWords:  EnsuresPresent EnsuresPresentIf TargetLocations

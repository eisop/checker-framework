...
The option is required.  Enabling \<eisopcf> without it reports a configuration error
rather than doing nothing, because a build that looks like it is type-checking and is not
is worse than one that fails:
\begin{Verbatim}
warning: [eisopcf] The eisopcf check is enabled but no Checker Framework checker is
selected, so nothing was type-checked. ...
\end{Verbatim}
To have the plugin on the processorpath without running it, turn the check off with
\<-Xep:eisopcf:OFF>.

...
Such an option will apply to a checker with that name, any subclass of that
checker, or any of its subcheckers (unless overridden by a more specific option).
...
downside when running independent checkers together is that, in this example, the Nullness Checker will issue a
...
However, when a checker has subcheckers (such as the Nullness Checker running \code{NullnessNoInitSubchecker}
and \code{KeyForSubchecker}), no such warning occurs: options prefixed with the parent checker's name automatically apply
to all subcheckers. A subchecker can also override the parent option with a more specific prefix:
\begin{Verbatim}
    -ANullnessChecker_lint=redundantNullComparison
    -ANullnessNoInitSubchecker_lint=-redundantNullComparison
\end{Verbatim}

\noindent
Checker-prefixed options always take precedence over contradictory unprefixed options:
\begin{Verbatim}
    -Alint=redundantNullComparison -ANullnessChecker_lint=-redundantNullComparison
\end{Verbatim}
...
\item \<-AuseConservativeDefaultsForUncheckedCode>
  ...
  \end{itemize}
\item \<-AusePermissiveDefaultsForUncheckedCode>
  Enables permissive defaults, and suppresses all type-checking warnings in unchecked source code.
  Takes arguments ``source,bytecode''; prefix an argument with ``-'' to disable it.
  Outside a relevant \refqualclass{framework/qual}{AnnotatedFor} scope, parameters default to
  top and returns and fields default to bottom.  Upper bounds default to top and lower bounds
  default to bottom.  See Sections~\ref{defaults-classfile} and~\ref{compiling-libraries}.
  A kind of code cannot use both permissive and conservative defaults.

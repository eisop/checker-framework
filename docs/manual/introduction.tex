...
\noindent
to pass different lint options to the Nullness and GUI Effect Checkers when run together.

When a checker has subcheckers (such as the Nullness Checker running \code{NullnessNoInitSubchecker}
and \code{KeyForSubchecker}), options prefixed with the parent checker's name automatically apply
to all subcheckers. A subchecker can override the parent option with a more specific prefix:
\begin{Verbatim}
    -ANullnessChecker_lint=redundantNullComparison

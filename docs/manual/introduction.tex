...
\item \<-AcheckPurityAnnotations>
  ...
  Section~\ref{type-refinement-purity}.
\item \<-AseparateBoxingArgument>
  Give the argument of a boxing conversion its own temporary variable in the
  control flow graph, so that the value before the conversion and the value
  after it are not conflated.  By default they share a tree, and the dataflow
  analysis can return the boxed value where the primitive one is meant.  This
  matters only to a type system in which boxing changes the qualifier; where the
  qualifier is preserved, the two values are the same.  The option makes
  boxing-heavy code measurably slower to check, so it is off by default.
...
% LocalWords:  AseparateBoxingArgument

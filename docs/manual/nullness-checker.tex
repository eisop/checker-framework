...
\item
  ...
  Finally, it writes only source-code annotations into the \<.class> file
  (Section~\ref{AnoBytecodeStorage}):  the qualifiers the checker infers by
  defaulting are not part of the JSpecify specification of the code, and storing
  them breaks downstream tools.
  ...
  -AnoBytecodeStorage -AassumeAssertions=enabled}.  All but the last are on/off flags with no negative form,

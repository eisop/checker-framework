...
\item
  ...
  Code outside the scope of a \code{@NullMarked} or \code{@AnnotatedFor} has what
  JSpecify calls unspecified nullness.  JSpecify leaves it to each tool how to treat
  unspecified nullness, from strict to lenient (see the
  \href{https://jspecify.dev/docs/spec/#multiple-worlds}{``multiple worlds'' discussion}
  in the JSpecify specification).  The mode currently interprets it leniently, with
  permissive defaults (Section~\ref{defaults-classfile}) for both source code and bytecode.
  ...
  -AusePermissiveDefaultsForUncheckedCode=source,bytecode}.  The first five are on/off
  flags with no negative form, so there is currently no way to enable this mode but turn
  one of them back off.
  The last two take a value, so writing one together with
  ...
  statements (Section~\ref{type-refinement-assertions}), and
  \code{-AusePermissiveDefaultsForUncheckedCode=-source,-bytecode} restores the ordinary
  defaults for unchecked code.  To use conservative defaults for unchecked code instead,
  write \code{-AuseConservativeDefaultsForUncheckedCode}, which replaces the mode's
  permissive defaults.

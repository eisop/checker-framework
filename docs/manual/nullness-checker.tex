...
\item
  ...
  checker will not recognize \code{org.jspecify.annotations.NullMarked}.
  By default it is recognized, both as a defaulting
  ...
  \code{@NullUnmarked} is not yet honored: nested code under it is still checked as if
  the enclosing \code{@NullMarked} scope applied.
...
\item
  ...
  an error for a nullness annotation written on a class declaration, on the return type
  of an annotation interface member, on a wildcard, before a type parameter, on a
  thrown type, on a pattern, on a type argument of a receiver parameter's type, or on
  the root type of a local variable, of a resource variable, of a cast, or of a
  method reference.  Each of these locations is reported under its own
  ...
  Two related locations are reported whether or not you supply this option, because the
  ...
  \code{instanceof.nullable} or \code{instanceof.nonnull.redundant}, and a component of

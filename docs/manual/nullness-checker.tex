...
\item
  ...
  checker will not recognize \code{org.jspecify.annotations.NullMarked}, nor its
  deprecated former location \code{org.jspecify.nullness.NullMarked}.
  By default both are recognized, both as a defaulting
  annotation and as an alias for
  \code{@AnnotatedFor(value = "nullness", applyToSubpackages = false)}, the latter so that
  \code{-AonlyAnnotatedFor} and
  \code{-AuseConservativeDefaultsForUncheckedCode=source} type-check \code{@NullMarked}
  code (Section~\ref{aonlyannotatedfor}).

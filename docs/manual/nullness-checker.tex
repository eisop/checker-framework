...
\item
  ...
  -AassumeAssertionsAreEnabled}.  Most of those are on/off flags with no negative form,
  so there is currently no way to enable this mode but turn one of them back off.  The
  exception is \code{-AassumeAssertionsAreEnabled}:  supplying
  \code{-AassumeAssertionsAreDisabled} together with \code{-Amode=jspecify} makes the
  checker assume that assertions are disabled instead.

...
in the same format as \refqualclass{framework/qual}{AnnotatedFor}'s, and it
has the same \<applyToSubpackages> element.  Between two package annotations,
the innermost one wins:  an \<@UnannotatedFor> on a package excludes its
subpackages from an \<@AnnotatedFor> on an enclosing package, and vice versa.

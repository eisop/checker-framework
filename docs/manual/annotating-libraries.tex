...
much faster than text-parsing the JDK\@.  Generate it by running
\<JavaStubifier> (Section~\ref{stub-creating-with-source}) over the annotated
JDK's root directory --- the directory that contains \<src>, not \<src> itself
--- which writes \<annotated-jdk.bin.gz> into that directory.  The Checker
Framework looks for the binary stub file within the same \<.jar> file that
supplies the annotated JDK, so a checker that distributes its own annotated JDK
uses its own binary stub file; the Checker Framework never substitutes the
binary stub file of a different annotated JDK for a missing one.
...
suppress that warning.  A stub file that a user supplies with \<-Astubs>, and an
\<.ajava> file, are text-parsed without a warning: they are not expected to have
a binary stub file.

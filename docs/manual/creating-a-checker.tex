...
\<SuggestedFix>, so it participates in Error Prone's patch workflow.  Because the
suggested-fix representation is host-agnostic, the checker's fix logic works with
any such host and the Checker Framework core does not depend on any of them.
...
When a finding has several independent fixes, pass them all to \code{withFixes} as
alternatives.  For a complete example, see

...
When specifying defaults procedurally via methods such as
\refmethod{framework/util/defaults}{QualifierDefaults}{addCheckedCodeDefault}{},
\refmethod{framework/util/defaults}{QualifierDefaults}{addUncheckedCodeDefault}{}, or
\refmethod{framework/util/defaults}{QualifierDefaults}{addElementDefault}{},
the target location must be permitted for the qualifier:
\begin{itemize}
\item Top and bottom qualifiers in a qualifier hierarchy are permitted as programmatic defaults at all locations by default (as the canonical bounds of the lattice), even if their \refqualclass{framework/qual}{TargetLocations} restricts explicit user use.
\item Other (intermediate) qualifiers must be permitted at the target location by their \refqualclass{framework/qual}{TargetLocations} meta-annotation, or they must be meta-annotated with \refqualclass{framework/qual}{ProgrammaticDefaultLocations} to explicitly permit programmatic defaulting at those locations.
\end{itemize}
If a programmatic default is registered for a location that is not permitted, the Checker Framework throws a \code{TypeSystemError}.
...
Restricting a qualifier with \refqualclass{framework/qual}{TargetLocations} prevents users from writing the qualifier explicitly at other locations in source code (and prevents user-written \refqualclass{framework/qual}{DefaultQualifier} from targeting them).
However, it does not prevent the type system or framework from using the top or bottom qualifier for programmatic defaults (such as conservative defaults for unchecked code or permissive defaults), because top and bottom qualifiers are inherently permitted for programmatic defaults.
If you define an intermediate qualifier that should be restricted from explicit use by users but permitted as a programmatic default, meta-annotate it with \refqualclass{framework/qual}{ProgrammaticDefaultLocations}.
...
\subsubsectionAndLabel{Checker-Specific Options}{creating-providing-command-line-options-checker-specific}

To direct a command-line option to a specific checker when multiple checkers are run together,
prefix the option name with the checker's simple or canonical class name followed by an underscore
``\<\_>''.
For example:
\begin{Verbatim}
...
\noindent
Options prefixed with an enclosing checker's name automatically apply to all of its subcheckers
(for example, options prefixed with \code{NullnessChecker\_} apply to \code{NullnessNoInitSubchecker}).
A subchecker can override a parent option with a more specific prefix
(e.g., \code{-ANullnessNoInitSubchecker\_lint=-redundantNullComparison}),
and checker-prefixed options always take precedence over contradictory unprefixed options.

Subcheckers automatically recognize options declared by their parent checkers.
For example, if an enclosing checker declares:
\begin{Verbatim}
@SupportedOptions({"myOption"})
public class MyParentChecker extends BaseTypeChecker { ... }
\end{Verbatim}

\noindent
any subchecker can query \code{checker.hasOption("myOption")} or \code{checker.getOption("myOption")}
directly without needing to inspect its parent checker.
The framework automatically resolves the value with the proper specificity precedence:
\begin{Verbatim}
    -AMySubchecker_myOption=val1    (highest precedence)
    -AMyParentChecker_myOption=val2
    -AmyOption=val3                 (lowest precedence)
\end{Verbatim}

\paragraphAndLabel{Option declarations in compound checkers and subcheckers}{creating-compound-checker-option-declarations}
Option declarations are shared bidirectionally between compound checkers and their subcheckers:
\begin{itemize}
  \item \textbf{Subchecker options inherited by parent checkers:} Any option declared by a subchecker via \code{@SupportedOptions} or \code{@SupportedLintOptions} is automatically recognized by enclosing parent checkers, and parent checkers automatically synthesize parent-prefixed forms (for example, \code{-AMyParentChecker\_mySubcheckerOption}). You do not need to repeat subchecker options on enclosing checkers.
  \item \textbf{Parent checker options inherited by subcheckers:} Subcheckers automatically recognize options declared on enclosing checkers and can query them directly.
  \item \textbf{Standalone execution:} Declaring options on the subchecker that queries them ensures that the subchecker can also be executed standalone as an independent annotation processor without throwing unrecognized option errors.
\end{itemize}
...
%%  LocalWords:  ProgrammaticDefaultLocations TypeUseLocation createExpressionAnnoHelper fromNode expr

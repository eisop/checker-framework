...
\noindent
Note that you can also convert the \<.dot> file to a \<.svg> file using
the \code{dot} program by using the command:
...
issues with the \code{com.sun} packages not being exported, add the \code{--add-exports} commands
found in \code{build.gradle} to the above command.

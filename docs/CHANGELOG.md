...
The Nullness Checker now recognizes references to private, final, zero-length field arrays in calls to `Collection.toArray(T[])`, allowing the return value to be refined to `@NonNull`.

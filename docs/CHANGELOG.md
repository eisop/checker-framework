...
The new command-line argument `-AaliasedTypeAnnos={aliases}` and `-AaliasedDeclAnnos={aliases}`
enables user to define custom type and declaraction annotations to be treat as

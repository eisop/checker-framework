...
public class QualifierDefaults {
  ...
  protected class DefaultApplierElementImpl extends AnnotatedTypeScanner<Void, AnnotationMirror> {
    ...
    public Void scan(@FindDistinct AnnotatedTypeMirror t, AnnotationMirror qual) {
      ...
      switch (outer.location) {
        case FIELD:
          if (outer.scope != null && outer.scope.getKind() == ElementKind.FIELD && isTopLevelType) {
        ...
        case EXPLICIT_UPPER_BOUND:
          if (isUpperBound
              && (boundType == BoundType.TYPEVAR_UPPER || boundType == BoundType.WILDCARD_UPPER)) {
            outer.addAnnotation(t, qual);
          }
          break;
        ...
        default:
          throw new BugInCF(
              "QualifierDefaults.DefaultApplierElement: unhandled location: " + outer.location);
      }
    ...
    @Override
    public Void visitTypeVariable(@FindDistinct AnnotatedTypeVariable type, AnnotationMirror qual) {
      ...
      boolean isLocalVariable = outer.scope != null && ElementUtils.isLocalVariable(outer.scope);
  ...
  /**
   * Returns the boundType (TYPEVAR_UPPER or TYPEVAR_UNBOUNDED) of the declaration of typeParamElem.
   *
   * @param typeParamElem the type parameter element
   * @return the boundType (TYPEVAR_UPPER or TYPEVAR_UNBOUNDED) of the declaration of typeParamElem
   */

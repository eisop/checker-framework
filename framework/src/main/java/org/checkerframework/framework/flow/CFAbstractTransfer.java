...
public abstract class CFAbstractTransfer<
  ...
  */
  ...
  private void addInitialFieldValues(S store, ClassTree classTree, MethodTree methodTree) {
    ...
    boolean isStaticMethod = ElementUtils.isStatic(TreeUtils.elementFromDeclaration(methodTree));
    ...
    for (Contract p : postconditions) {
      ...
      try {
        ...
        if (e.isFlowParseError()) {
          ...
          args[0] =
              ElementUtils.getSimpleSignature((ExecutableElement) TreeUtils.elementFromUse(tree));

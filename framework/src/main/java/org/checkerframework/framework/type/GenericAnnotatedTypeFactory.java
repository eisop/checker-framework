...
public abstract class GenericAnnotatedTypeFactory<
  ...
  */
  ...
  protected void analyze(
    ...
    ControlFlowGraph cfg = CFCFGBuilder.build(this.getRoot(), ast, checker, this, processingEnv);
    /*
  ...
  /**
   * Whether {@link #getAnnotatedTypeLhs(Tree)} is running right now. This can be used by subclasses
   * whenever the type of expression differs depending on whether it occurs on the left- or
   * right-hand side of an assignment (e.g., in the presence of type refinements or for
   ...
   * hierarchies' qualifiers are changed based on the receiver type and the declared annotations for
   * a field.
   *
   ...
   * tree}.
   *
   * <p>This is used by {@link BaseTypeVisitor#visitMethodInvocation(MethodInvocationTree, Void)} to
   * check the preconditions of method calls.
   *
  ...
  public AnnotatedTypeMirror getAnnotatedTypeLhs(Tree lhsTree) {
    ...
    switch (lhsTree.getKind()) {
      case VARIABLE:
        boolean isVarTree = TreeUtils.isVariableTreeDeclaredUsingVar((VariableTree) lhsTree);
        if (isVarTree) {
  ...
  /**
   * As long as everUseFlow is true, always enable flow refinement for the receiver. This method is
   * an implementation detail and visible inside the package only. See the comment in this testcase
   * for more details framework/tests/viewpointtest/TestGetAnnotatedLhs.java.
   *
   ...
   */
  protected final void addComputedTypeAnnotationsWithoutFlow(Tree tree, AnnotatedTypeMirror type) {
    boolean oldUseflow = useFlow;
    useFlow = false;

...
public abstract class AbstractViewpointAdapter implements ViewpointAdapter {
   ...
   * type variable if it is an upper bound of another type variable. We only viewpoint adapt a type
   * variable that is not an upper-bound.
   */
  ...
  public void viewpointAdaptMember(
      AnnotatedTypeMirror receiverType, Element memberElement, AnnotatedTypeMirror memberType) {
    ...
    if (memberType.getKind() == TypeKind.DECLARED && combinedType.getKind() == TypeKind.DECLARED) {
    ...
    } else if (memberType.getKind() == TypeKind.ARRAY && combinedType.getKind() == TypeKind.ARRAY) {
   ...
   *
   * @param type type of the member, used to decide whether a member should be viewpoint adapted or
   *     not. A subclass of {@link ViewpointAdapter} may disable viewpoint adaptation for elements
  ...
  public void viewpointAdaptConstructor(
    ...
    IdentityHashMap<AnnotatedTypeMirror, AnnotatedTypeMirror> mappings = new IdentityHashMap<>();
    ...
    unsubstitutedConstructorType =
        (AnnotatedExecutableType)
            AnnotatedTypeCopierWithReplacement.replace(unsubstitutedConstructorType, mappings);
  ...
  public void viewpointAdaptMethod(
    ...
    unsubstitutedMethodType =
        (AnnotatedExecutableType)
            AnnotatedTypeCopierWithReplacement.replace(unsubstitutedMethodType, mappings);
  ...
  public void viewpointAdaptTypeParameterBounds(
      AnnotatedTypeMirror receiverType, List<AnnotatedTypeParameterBounds> typeParameterBounds) {
    ...
    for (AnnotatedTypeParameterBounds typeParameterBound : typeParameterBounds) {
      ...
      adaptedTypeParameterBounds.add(new AnnotatedTypeParameterBounds(adaptedUpper, adaptedLower));
    }
  ...
  protected AnnotatedTypeMirror combineAnnotationWithType(
    ...
    if (declared.getKind().isPrimitive()) {
      ...
      AnnotationMirror resultAnnotation =
          combineAnnotationWithAnnotation(receiverAnnotation, extractAnnotationMirror(apt));
      ...
      if (!isTypeVarExtends) {
        ...
        AnnotatedTypeMirror result = AnnotatedTypeCopierWithReplacement.replace(atv, mappings);
      ...
      AnnotationMirror resultAnnotation =
          combineAnnotationWithAnnotation(receiverAnnotation, extractAnnotationMirror(adt));
      ...
      AnnotationMirror resultAnnotation =
          combineAnnotationWithAnnotation(receiverAnnotation, extractAnnotationMirror(aat));
      ...
      AnnotatedTypeMirror combinedCompoType = combineAnnotationWithType(receiverAnnotation, compo);
      ...
      if (extend != null) {
        ...
        AnnotatedTypeMirror combinedExtend = combineAnnotationWithType(receiverAnnotation, extend);
      ...
      if (zuper != null) {
        ...
        AnnotatedTypeMirror combinedZuper = combineAnnotationWithType(receiverAnnotation, zuper);
      ...
      AnnotationMirror resultAnnotation =
          combineAnnotationWithAnnotation(receiverAnnotation, extractAnnotationMirror(ant));

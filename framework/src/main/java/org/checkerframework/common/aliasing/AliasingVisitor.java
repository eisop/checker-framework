...
public class AliasingVisitor extends BaseTypeVisitor<AliasingAnnotatedTypeFactory> {
    ...
    public Void visitMethodInvocation(MethodInvocationTree node, Void p) {
        ...
        if (isInUniqueConstructor()) {
            if (TreeUtils.isSuperConstructorCall(node)) {
                ...
                // constructor
                // result, therefore we need this "else" block. Once constructors are implemented
                // correctly
                // we could remove that code below, since the type of "this" in a @Unique
                // constructor will
                // be @Unique.
    ...
    // raise a unique.leaked warning, as there is a pseudo-assignment from @Unique to a
    // @MaybeAliased

...
public class BaseTypeValidator extends AnnotatedTypeScanner<Void, Tree> implements TypeValidator {
  ...
  protected List<DiagMessage> isValidStructurally(AnnotatedTypeMirror type) {
    SimpleAnnotatedTypeScanner<List<DiagMessage>, Void> scanner =
        new SimpleAnnotatedTypeScanner<>(
            (atm, p) -> isTopLevelValidType(atm), DiagMessage::mergeLists, Collections.emptyList());
    ...
    for (int i = 0; i < numTypeArgs; i++) {
      ...
      if (TypesUtils.isCapturedTypeVariable(captureTypeArg.getUnderlyingType())) {
        ...
        if (!(atypeFactory
            ...
            && atypeFactory
                .getTypeHierarchy()
                .isSubtypeShallowEffective(wildcard.getExtendsBound(), wildcard.getSuperBound()))) {
          checker.reportError(
              tree.getTypeArguments().get(i),
   ...
   * org.checkerframework.framework.qual.TargetLocations}. Report an error if the actual use of this
   * annotation is not listed in the declared TypeUseLocations in this meta-annotation.
   *
   * @param type the type to check

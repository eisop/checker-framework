...
public enum TypeUseLocation {
  ...
  /**
   * Apply default annotations to unannotated top-level explicit type parameter upper bounds: {@code
   * <T extends C>}.
   */
  ...
  /**
   * Apply default annotations to unannotated upper bounds of type parameters and wildcards without
   * explicit upper bounds: {@code <T>}, {@code <?>}, and {@code <? super C>}.
   */
  IMPLICIT_UPPER_BOUND,

  /**
   * Apply default annotations to unannotated upper bounds of type parameters without explicit upper
   * bounds: {@code <T>}.
   */
   ...
   * introduce a separate constant {@code TYPE_VARIABLE_USE_TOP_LEVEL_LOCAL_VARIABLE}. At the moment
   * we use the {@code LOCAL_VARIABLE} default for unannotated top-level type variable uses of local
   * variables: {@code T local}.
   */
  TYPE_VARIABLE_USE,

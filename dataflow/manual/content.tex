...
\subsubsection{UnusedAbsValue}
\label{sec:UnusedAbsValue_classes}

UnusedAbsValue, as the name suggests, is an AbstractValue but is
not involved in any computation during dataflow analysis. That is, the
UnusedAbsValue implements the leastUpperBound method in a way it
just throws an error if it is called, without computing the lub with the
other AbstractValue.

It can be used in an analysis which does not require any computation
between two AbstractValues. For example, in LiveVariable analysis (\autoref{sec:live_variable}), we
need to compute the lub of successors' stores, and it is meaningless to
further calculate the lub of several live variables in a smaller granularity.
...
update the values in the stores.

...
\section{Example: Reaching Definitions}

The reaching definitions for a given program point are those assignments that
may have defined the current values of variables.

A reaching definitions analysis for definitions(assignments) was implemented
to show the forward analysis works properly.

\textbf{Abstract values.} A class \code{ReachingDefinitionsValue} is a reaching
definitions (which is represented by a node) wrapper turning node into
abstract value. A node can only be \code{AssignmentNode}.

\textbf{The store.} The reaching definitions store \code{ReachingDefinitionsStore} has a field
\code{reachingDefSet} which contains a set of \code{ReachingDefinitionsValue}s. Only
\code{AssignmentNode} will be considered as a
definition and added to \code{reachingDefSet}. The store defines methods
\code{putDef(ReachingDefinitionValue)} and \code{killDef(Node)} to add
and kill reaching definitions. A definition will be added in the store when the analysis visits
an \code{AssignmentNode}, and will be killed if afterwards the analysis visits a new
\code{AssignmentNode} with the same lhs.

\textbf{The transfer function.} The transfer function \code{ReachingDefinitionTransfer}
initializes empty store. The transfer function visits assignments to
update the information of reaching definitions for each node in the stores.

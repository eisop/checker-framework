...
live variable and addedc to \code{liveVarValueSet}. The store defines methods
...
Very busy expression analysis is a backward analysis that intersects abstract states from successors.
An expression is very busy if no matter what path is taken, the expression is used before any variables
occurring in it are redefined.
\\
The main class is \code{org.checkerframework.dataflow.cfg.playground.BusyExpressionPlayground}.

\textbf{Abstract values.} The \code{BusyExprValue} is a very busy expression
(which is represented by a node) wrapper turning node into
abstract value. The node can be any \code{BinaryOperationNode}, such as \code{NumericalAdditionNode}
...
If a node is \code{BinaryOperationNode}, \code{addUseInExpression(Node)}
will recursively analyze the rhs and lhs of the node to determine
...
return statement to update the busy expression values in the stores.

\textbf{Example.} An example is shown in \autoref{fig:BusyExprSimple}.

\flow{BusyExprSimple}{.36}{1}{Very Busy Expression Analysis illustration. Intermediate analysis results are shown.}

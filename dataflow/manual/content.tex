...
\section{Example: Reach Definition}

A reach definition analysis for definitions(assignments) was implemented
to show the forward analysis works properly. The main class is
\code{org.checkerframework.dataflow.cfg.playground.ReachDefinitionPlayground}.

\textbf{Abstract values.} A class \code{ReachDefinitionValue} is a reach
definition (which is represented by a node) wrapper turning node into
abstract value. A node can only be \code{AssignmentNode}.

\textbf{The store.} The reach definition store \code{ReachDefinitionStore} has a field
\code{reachDefSet} which contains a set of \code{ReachDefinitionValue}s. Only
\code{AssignmentNode} will be considered as a
definition and added to \code{reachDefSet}. The store defines methods
\code{putDef(ReachDefinitionValue)} and \code{killDef(Node)} to add
and kill reach definitions.

\textbf{The transfer function.} The transfer function \code{ReachDefinitionTransfer}
initializes empty store. The transfer function visits assignments to
update the information of reach definitions for each node in the stores.

\textbf{Example.} An example is shown in \autoref{fig:ReachSimple}.

\flow{ReachSimple}{.33}{1}{Simple sequential program to illustrate reach definitions.  Intermediate analysis results are shown.}

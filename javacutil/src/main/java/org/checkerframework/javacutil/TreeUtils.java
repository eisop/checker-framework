...
public final class TreeUtils {
  ...
  public static List<? extends CaseTree> switchExpressionTreeGetCases(Tree switchExpressionTree) {
    if (atLeastJava12) {
      try {
        ...
        throw new BugInCF(
            "switchExpressionTreeGetCases: cases is null for tree: %s", switchExpressionTree);
      } catch (InvocationTargetException | IllegalAccessException e) {
        throw new BugInCF(
  ...
  public static ExpressionTree yieldTreeGetValue(Tree yieldTree) {
    if (atLeastJava13) {
      try {
        @SuppressWarnings("nullness")
        ExpressionTree expressionTree = (ExpressionTree) YIELDTREE_GETVALUE.invoke(yieldTree);
        if (expressionTree != null) {
          return expressionTree;
        }

 ...
 *   either.  Every compilation below is checked against a .out file, so the test fails if the
 *   diagnostics change, not only if a compilation crashes.  See eisop issue 2094 and typetools
 *   issue 8055.
 *
 *   The crash was two bugs, one behind the other.  First,
 ...
 * @compile/ref=Ok.out -XDrawDiagnostics -processor org.checkerframework.checker.tainting.TaintingChecker Ok.java
 * @compile/ref=ClassLiteral.out -XDrawDiagnostics -processor org.checkerframework.checker.tainting.TaintingChecker ClassLiteral.java
 * @compile/ref=ClassLiteralNullness.out -XDrawDiagnostics -processor org.checkerframework.checker.nullness.NullnessChecker ClassLiteral.java
 * @compile/ref=Field.out -XDrawDiagnostics -processor org.checkerframework.checker.tainting.TaintingChecker Field.java
 * @compile/ref=MethodCall.out -XDrawDiagnostics -processor org.checkerframework.checker.tainting.TaintingChecker MethodCall.java
 * @compile/ref=Parameter.out -XDrawDiagnostics -processor org.checkerframework.checker.tainting.TaintingChecker Parameter.java
 * @compile/fail/ref=InheritedAnno.out -XDrawDiagnostics -processor org.checkerframework.checker.tainting.TaintingChecker InheritedAnno.java
 */
 ...
 *
 * Every @compile names a .out file, because a bare @compile checks only the exit status: it would
 * pass whether the warning is issued once, five times, or not at all.  -XDrawDiagnostics is what
 * makes the reference files portable; without it a diagnostic that has a source position prints the

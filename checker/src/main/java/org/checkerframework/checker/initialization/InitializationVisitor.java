...
public class InitializationVisitor extends BaseTypeVisitor<InitializationAnnotatedTypeFactory> {
   ...
   * Object...)} to get the correct store value for the right-hand side's fields and checker whether
  ...
  protected void reportMethodInvocabilityError(
    ...
    List<VariableTree> uninitializedFields =
        atypeFactory.getUninitializedFields(
            atypeFactory.getStoreBefore(node), getCurrentPath(), false, Collections.emptyList());
    uninitializedFields.removeAll(initializedFields);
    ...
    if (!uninitializedFields.isEmpty()) {
      ...
      checker.reportError(
          node, "method.invocation.invalid", new Object[] {found.toString(), expected.toString()});
    }
  }
  ...
  protected void filterInitializedFields(
    ...
    uninitializedFields.removeIf(
        var -> {
          ...
          return InitializationAnnotatedTypeFactory.isInitialized(factory, value, varElement);
        });
  }
}

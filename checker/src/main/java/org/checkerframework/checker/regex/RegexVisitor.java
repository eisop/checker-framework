        ...
        if (TreeUtils.isMethodInvocation(node, patternCompile, env)) {
            ...
            if (flagParam.getKind() == Kind.MEMBER_SELECT) {
                ...
                if (TreeUtils.isSpecificFieldAccess(memSelect, patternLiteral)) {
                    ...
                    // parameter
                    // doesn't need to be a @Regex String. Don't call the super method to skip
                    // checking if the
                    // first parameter is a @Regex String, but make sure to still recurse on all of
                    // the
            ...
            if (group.getKind() == Kind.INT_LITERAL) {
                ...
                if (receiver == null) {
                    ...
                    // group (and
                    // other methods) don't have a receiver tree.  So, just do the regular checking.
                    // Verifying
                    // an implemenation of a subclass of MatcherResult is out of the scope of this
                    // checker.

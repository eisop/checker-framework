...
public class NullnessNoInitAnnotatedTypeFactory
    extends GenericAnnotatedTypeFactory<
        NullnessNoInitValue, NullnessNoInitStore, NullnessNoInitTransfer, NullnessNoInitAnalysis> {
  ...
  protected Set<Class<? extends Annotation>> createSupportedTypeQualifiers() {
    return new LinkedHashSet<>(
        Arrays.asList(Nullable.class, MonotonicNonNull.class, NonNull.class, PolyNull.class));
  }
  ...
  public AnnotatedTypeMirror getAnnotatedTypeBefore(JavaExpression expr, ExpressionTree tree) {
    ...
    if (expr instanceof FieldAccess) {
      ...
      if (initFactory.isInitializedForFrame(receiverType, declaringClass)) {
        ...
        res.addAnnotations(
            qualHierarchy.greatestLowerBounds(declared.getAnnotations(), refined.getAnnotations()));
        return res;
      }
    }
  ...
  protected class NullnessPropagationTreeAnnotator extends PropagationTreeAnnotator {
    ...
    public Void visitTypeCast(TypeCastTree tree, AnnotatedTypeMirror type) {
      if (type.getKind().isPrimitive()) {
        AnnotationMirror NONNULL = ((NullnessNoInitAnnotatedTypeFactory) atypeFactory).NONNULL;
